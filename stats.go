...
func fillCommits(email string, path string, commits map[int]int) map[int]int {
	...
	if err != nil {
		fmt.Printf("Warning: Could not open repository at %s: %v\n", path, err)
		return commits
	}
	...
	ref, err := repo.Head()
	if err != nil {
		fmt.Printf("Warning: Could not get HEAD reference for repository at %s: %v\n", path, err)
		return commits
	}
	...
	if err != nil {
		fmt.Printf("Warning: Could not get commit log for repository at %s: %v\n", path, err)
		return commits
	}
	...
	if err != nil {
		fmt.Printf("Warning: Error iterating commits for repository at %s: %v\n", path, err)
	}
...
func processRepositories(email string) map[int]int {
	...
	if len(repos) == 0 {
		fmt.Println("No repositories found. Please add repositories using -add flag first.")
		return commits
	}
